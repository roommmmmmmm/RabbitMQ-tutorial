...
class Ini extends Config
{
    ...
    public function __construct($filename, $section = null, $options = false)
    {
        if (empty($filename)) {
            ...
            throw new ZendException('Filename is not set');
        }
        ...
        if (null === $section) {
            ...
            foreach ($section as $sectionName) {
                if (!isset($iniArray[$sectionName])) {
                     ...
                     */
                    //require_once 'Zend/Config/Exception.php';
                    throw new ZendException("Section '$sectionName' cannot be found in $filename");
                }
    ...
    protected function _parseIniFile($filename)
    {
        ...
        if ($this->_loadFileErrorStr !== null) {
             ...
             */
            //require_once 'Zend/Config/Exception.php';
            throw new ZendException($this->_loadFileErrorStr);
        }
        ...
        foreach ($loaded as $key => $data)
        {
            ...
            switch (count($pieces)) {
                ...
                default:
                     ...
                     */
                    //require_once 'Zend/Config/Exception.php';
                    throw new ZendException("Section '$thisSection' may not extend multiple sections in $filename");
            }
        }
        ...
        foreach ($thisSection as $key => $value) {
            if (strtolower($key) == ';extends') {
                if (isset($iniArray[$value])) {
                     ...
                     */
                    //require_once 'Zend/Config/Exception.php';
                    throw new ZendException("Parent section '$section' cannot be found");
                }
            } else {
        ...
        if (strpos($key, $this->_nestSeparator) !== false) {
            ...
            if (strlen($pieces[0]) && strlen($pieces[1])) {
                if (!isset($config[$pieces[0]])) {
                     ...
                     */
                    //require_once 'Zend/Config/Exception.php';
                    throw new ZendException("Cannot create sub-key for '{$pieces[0]}' as key already exists");
                }
                 ...
                 */
                //require_once 'Zend/Config/Exception.php';
                throw new ZendException("Invalid key '$key'");
            }
        } else {
            $config[$key] = $value;
        }
